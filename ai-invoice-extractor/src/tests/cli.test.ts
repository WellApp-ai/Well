...
describe("cli", () => {
  it("should exit with an error if the file path is missing", async () => {
    ...
    expect(stderr.join("\n")).toContain("error: missing required argument 'file-path'")
  })
  ...
  it("should exit with an error if the AI API key is not provided", async () => {
    ...
    expect(exitCode).toBe(1)
    expect(stderr.join("\n")).toContain("No AI configuration found. Please provide an API key.")
  })
  ...
  it("should call the extractor with the correct parameters", async () => {
    ...
    await main(["--vendor", "openai", "--model", "gpt-4", "--key", "test-key", "file.pdf"])
  ...
  it("should output pretty JSON when the --pretty option is used", async () => {
    const { stdout } = await main(["--vendor", "openai", "--key", "test-key", "--pretty", "file.pdf"])
    expect(stdout.join("\n")).toContain(JSON.stringify({ success: true }, null, 2))
  })

  it("should output compact JSON by default", async () => {
    const { stdout } = await main(["--vendor", "openai", "--key", "test-key", "file.pdf"])
  ...
  it("should handle extraction errors gracefully", async () => {
    ...
    const { exitCode, stderr } = await main(["--vendor", "openai", "--key", "test-key", "file.pdf"])
    // Exit 1 is reserved for caller errors; a failure inside extraction is 2.
